...
export default {
  ...
  async link() {
    ...
    let linkFormat = `(\\^${this.linkIdFormat}\\^)|(^#.*\\^${this.linkIdFormat}\\^)|(\\[[^\\]]+\\]\\(\\^${this.linkIdFormat}\\^\\))|(\\[[^\\]]*\\^${this.linkIdFormat}\\^[^\\]]*\\]\\([^\\)]+\\))|(\\[\\[[A-Za-z0-9]+\\]\\])`
    let linkRe = new RegExp(linkFormat)
    let currentRange = cursor.getCurrentWordBufferRange({ wordRegex: linkFormat })
    let linkIdRe = new RegExp(`\\^(${this.linkIdFormat})\\^|\\[\\[([A-Za-z0-9]+)\\]\\]`, 'g')
    let matches = linkIdRe.exec(editor.getTextInBufferRange(currentRange))
    if (matches) {
      linkId = matches[1]
      if (!linkId) linkId = matches[2]
      // A link ID is under or near the cursor, go on to the next one
      await this.linkIdModel.selectNext(linkId, editor, currentRange)
    } else {
      this.insertAtSelection(await this.linkIdModel.createUnique())
    }
  },

  async insertAtSelection(linkId) {
      let editor = atom.workspace.getActiveTextEditor()
      let cursor = editor.getLastCursor()
      if (linkText = editor.getLastSelection().getText()) {
          ...
          let currentRange = cursor.getCurrentWordBufferRange({ wordRegex: new RegExp(`\\[[^\\]]+\\]\\(\\)`) })
          if (!currentRange.isEmpty()) {
              editor.setSelectedBufferRange([currentRange.end, currentRange.end])
              editor.moveLeft(1)
          } else if (cursor.isInsideWord() && !cursor.isAtEndOfLine()) {
              cursor.moveToEndOfWord()
              editor.insertText(' ')
          }
      }
      let at = editor.getLastSelection().getBufferRange().end
  ...
  async insertLast() {
    ...
    this.insertAtSelection(lastLinkId)
  }
};
