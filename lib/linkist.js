  ...
  link() {
    ...
    async function findNext(tag) {
      ...
      matches.forEach(fileMatch => {
        fileMatch.matches.forEach(itemMatch => {
          ...
          if (fileMatch.filePath == editor.getPath() && matchRange.intersectsWith(foundRange)) {
            found = true
          } else {
    ...
    async function selectNext(tag) {
      ...
      if (tagAt) {
        ...
        editor.setSelectedBufferRange(foundRange, {flash: true})
      }
    }
    ...
    let tagFormat = `[${this.linkDigits}]{3,}`
    let tagRe = new RegExp(tagFormat, 'g')
    let linkFormat = `(\\[[^\\]]+\\])?\\(${tagFormat}\\)|\\[${tagFormat}\\]|\\^${tagFormat}\\^`
    let linkRe = new RegExp(linkFormat)
    let foundRange = cursor.getCurrentWordBufferRange({ wordRegex: linkFormat })
    let cursorText = editor.getTextInBufferRange(foundRange)
    let matches = cursorText.match(tagRe)
    if (!matches) {
      // No tag under cursor, create and insert a tag
      ...
      selectNext(matches[matches.length - 1])
    }
  }
};
